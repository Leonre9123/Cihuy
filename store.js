...
module.exports = alpha = async (alpha, m, chatUpdate, store, opengc, setpay, antilink, antiwame, antilink2, antiwame2, set_welcome_db, set_left_db, set_proses, set_done, set_open, set_close, sewa, _welcome, _left, db_respon_list) => {
    try {
        ...
        const isCreator = ["62888070349807@s.whatsapp.net",botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
if(m.isGroup){
    expiredCheck(alpha, sewa)
      ...
      if (isAntiLink) {
        ...
        m.reply(`*「 ANTI LINK 」*\n\nLink Grup Lain Terdeteksi, maaf pesan kamu akan dihapus`)
...
await alpha.sendMessage(m.chat, {
        ...
        alpha.groupParticipantsUpdate(m.chat, [m.sender], 'delete')
        }
        }
      if (isAntiLink2) {
        if (budy.match(`chat.whatsapp.com`)) {
...
await alpha.sendMessage(m.chat, {
      ...
      if (isAntiWame) {
        if (budy.match(`wa.me/`)) {
        m.reply(`*「 ANTI WA ME 」*\n\nWa Me detected, maaf pesan kamu akan dihapus`)
        if (!isBotAdmins) return m.reply(`Upsss... gajadi, bot bukan admin`)
await alpha.sendMessage(m.chat, {
               delete: {
                  remoteJid: m.chat,
      ...
      if (isAntiWame) {
        ...
        switch(command) {
         ...
         break
          ...
          break
          case "payment": case 'qris': case 'pay':{
...
let dwnld = await alpha.downloadMediaMessage(quoted)
